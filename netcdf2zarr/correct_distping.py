...
def widen_gaps(A, gap_width):
    # Count the number of NaN values
    nan_count = np.sum(np.isnan(A))
    # If the number of NaN values is 10 or more, widen the gaps
    if nan_count >= 10:
        # Identify NaN regions
        nan_indices = np.isnan(A)
        # Expand NaN regions by gap_width
...
def fill_nan2(A, gap_width=1):
    # Widen gaps if there are 10 or more NaN values
    widen_gaps(A, gap_width)
    ...
    # Calculate number of filled cells and difference in values in the gaps
    filled_cells = np.sum(np.isnan(A)) - np.sum(np.isnan(B))
    gap_difference = np.nanmean(B) - np.nanmean(A)
    print("Filled array:", B)
    print("Number of filled cells:", filled_cells)
    print("Difference in values in the gaps:", gap_difference)

    return B

def isNaN(num):
    result=False
    if  isinstance(num, (int, float,numpy.int64 )) :
        result=math.isnan(num)
        #print (result)
        #print (num)
        #print ("---")

    elif  isinstance( num, pd._libs.tslibs.timestamps.Timestamp)  :
        result=False
    else:
        result=True
    if result:
...
def correct_parquet(parquetfile):
    ...
    pings2 = fill_nan2(pings2, gap_width=1)
