...
if local==1:
    ...
    f = '/data/crimac-scratch/2025/OF_Conversion/temp_SalmonTest.zarr'
...
print("data['ping_time']: ", ping_times)
...
print(type(sv_sel), sv_sel.dims)

sv_sel = sv_sel.isel(frequency=0)  # select a single frequency (1st frequency)
...
# print("***********************************************")
# print(type(sv_sel_db))
# Create plot

# Ensure coordinates are sorted
sv_sel_db = sv_sel_db.sortby('ping_time').sortby('range')

# Convert ping_time to numeric (needed for Holoviews Image)
import matplotlib.dates as mdates
x = mdates.date2num(sv_sel_db['ping_time'].values)
y = sv_sel_db['range'].values
z = sv_sel_db.values

# Make sure z is 2D with shape (len(y), len(x))
if z.shape != (len(y), len(x)):
    z = z.T

plot = hv.Image((x, y, z), kdims=['ping_time', 'range'], vdims='Sv').opts(
    cmap='viridis',
    invert_yaxis=True,
    width=2000,
    height=800,
...
# Render and save static image
static_plot = hv.render(plot, backend='matplotlib')
static_plot.savefig('sv_70kHz.png', dpi=200)
...
# Render the DynamicMap to a static element
